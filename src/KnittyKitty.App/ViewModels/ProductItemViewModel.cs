...
public sealed class ProductItemViewModel : ObservableObject
{
    ...
    {
        Product = product;
        Image = CreateImage(product.ImagePath);
    ...
    {
        Image = CreateImage(ResolveImagePath());
    }
    ...
    /// Создаёт BitmapImage для карточки товара из локального пути.
    private static BitmapImage? CreateImage(string imagePath)
    {
        ...
        var fullPath = Path.Combine(AppContext.BaseDirectory, imagePath.Replace('/', Path.DirectorySeparatorChar));
