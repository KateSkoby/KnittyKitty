...
Самый простой способ проверить проект без ручной сборки — открыть вкладку **Releases** в GitHub-репозитории и скачать последнюю опубликованную версию приложения. В релиз рекомендуется прикладывать архив с готовой WinUI-сборкой и, при необходимости, отдельный архив с WebAssembly-версией.

Для запуска Веб-версии достаточно перейти по ссылке: https://kateskoby.github.io/KnittyKitty/

После скачивания релиза нужно распаковать архив и запустить исполняемый файл WinUI-приложения. Если релиз содержит WebAssembly-сборку, ее можно открыть через локальный сервер или опубликованный адрес, указанный в описании релиза.

Для запуска проекта из исходного кода нужен установленный .NET SDK 10 и workloads для WinUI/Uno Platform.
